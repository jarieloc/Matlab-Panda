...
for i = 1:1
    pseg = []
    px = double(subs(a_mat_1 * cart_vec, [p0,pf], [0.6, 0.6]))
    py = double(subs(a_mat_1 * cart_vec, [p0,pf], [1, 0.5]))
    pseg = [px';py']
    pseg_collect{i} = pseg
end
...
%     coord_nr=1;
    for i = 1:2
        ...
        X2(j,i)=pseg(i,1)*t^5+pseg(i,2)*t^4+pseg(i,3)*t^3+pseg(i,4)*t^2+pseg(i,5)*t+pseg(i,6);
    end
end

X2 = [X2, zeros(length(plen),1)]


X2dot = gradient(X2',step_size)'
X2ddot = gradient(X2dot',step_size)'
